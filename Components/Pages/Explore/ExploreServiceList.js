...
const ExploreServiceList = ({serviceList, categories, subCategories, loading, setServiceList}) => {
  ...
  return (
    <View>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ display : 'flex', flexDirection : 'row',flexWrap : 'wrap', justifyContent : 'space-between', rowGap : 15, flex : 1}} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>handleRefresh()} />}>
        
    {
      ...
      <ScrollView showsHorizontalScrollIndicator={false} contentContainerStyle={{display : 'flex', flexDirection : 'row', flexWrap : 'wrap'}} className="gap-3">
        ...
        serviceList?.map((service)=>{
            return (
                <View key={service._id} className="w-[48%] flex flex-col justify-start relative overflow-hidden bg-white rounded-md shadow-sm">
