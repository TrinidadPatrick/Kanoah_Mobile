...
const BlockedServices = () => {
    ...
    useEffect(()=>{
        const getDoNotShow = async () => {
            const accessToken = await SecureStore.getItemAsync('accessToken')
            try {
                const result = await http.get('Mobile_getDoNotShow', {
                    headers : {
                        "Content-Type" : 'aplication/json',
                        "Authorization" : `Bearer ${accessToken}`,
                    }
                })
                setBlockedServices(result.data)
            } catch (error) {
                console.log(error)
            }
        }
    ...
    const handleSort = (option) => {
        switch (option) {
            ...
            const sorted1 = blockedServices?.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt))
            setBlockedServices(sorted1)
            setShowSort(false)
            setSelectedSort(option)
            break;
            case "Date added(oldest)":
            const sorted2 = blockedServices?.sort((a,b) => new Date(a.createdAt) - new Date(b.createdAt))
            setBlockedServices(sorted2)
            setShowSort(false)
            setSelectedSort(option)
            break;
            case "Date published(newest)":
            const sorted3 = blockedServices?.sort((a,b) => new Date(b.service.createdAt) - new Date(a.service.createdAt))
            setBlockedServices(sorted3)
            setShowSort(false)
            setSelectedSort(option)
            break;
            case "Date published(oldest)":
            const sorted4 = blockedServices?.sort((a,b) => new Date(a.service.createdAt) - new Date(b.service.createdAt))
            setBlockedServices(sorted4)
            setShowSort(false)
            setSelectedSort(option)
            break;
    ...
    const removeDNS = async (serviceId) => {
        const DNSInstance = [...blockedServices]
        const index = DNSInstance.findIndex((dns) => dns.service._id === serviceId)
        DNSInstance.splice(index, 1)
        setBlockedServices(DNSInstance)
        try {
            const accessToken = await SecureStore.getItemAsync('accessToken')
          const result = await http.delete(`Mobile_removeDoNotShow/${serviceId}`, {
            headers : {
  ...
  return (
    ...
    <View className="flex-1 bg-[#f9f9f9] flex-col">
        ...
        <View className="w-full flex-row px-2 pt-3 items-center gap-x-1 relative">
            ...
            <Text className="font-medium text-gray-500">Showing {blockedServices === null ? 0 : blockedServices?.length} results</Text>
                ...
                return (
                    <View key={service._id} className="flex-row gap-x-2 bg-white p-2 overflow-visible shadow-2xl relative">
                        ...
                        <View className={`${selectedItemId === item._id ? "" : "hidden"} absolute right-6 bottom-0 border border-gray-100 z-20 bg-white shadow-xl rounded-md flex-col`}>
                            <TouchableOpacity onPress={()=>removeDNS(item.service._id)} className="py-2 px-2">
